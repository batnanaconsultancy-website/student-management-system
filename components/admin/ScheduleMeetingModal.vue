...
const dayItems = DAY_OPTIONS.map((d) => ({
  label: d.label,
...
const dayOfWeekStr = computed({
  ...
  set: (val: string) => {
    formData.value.day_of_week = Number(val);
  },
});
...
function validateForm() {
  ...
  if (
    formData.value.ends_on &&
    formData.value.ends_on < formData.value.starts_on
  ) {
...
async function handleSubmit() {
  ...
  try {
    emit("submit", {
      ...
      program_id:
        formData.value.program_id === "all" ? null : formData.value.program_id,
      cohort_id:
        formData.value.cohort_id === "all" ? null : formData.value.cohort_id,
    });
  } finally {
    loading.value = false;
  }
    ...
    <UModalContent>
      <UModalHeader>
        <div class="flex items-center gap-3">
          <div
            class="bg-primary/10 border-primary flex h-10 w-10 items-center justify-center rounded-full border"
          >
          ...
          <div>
            ...
            <p class="text-sm text-muted">
              Pins automatically to every matching student's calendar
            </p>
          </div>
        </div>
      </UModalHeader>
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <div class="space-y-2">
            ...
            <UInput
              ...
              :disabled="loading"
            />
            <p v-if="errors.title" class="text-xs text-error">
              {{ errors.title }}
            </p>
          </div>
          ...
          <div class="grid grid-cols-3 gap-3">
            <div class="space-y-2">
              <label class="text-sm font-medium text-highlighted"
                >Day <span class="text-error">*</span></label
              >
              <USelect
            ...
            <div class="space-y-2">
              <label class="text-sm font-medium text-highlighted"
                >Start <span class="text-error">*</span></label
              >
              <UInput
            ...
            <div class="space-y-2">
              <label class="text-sm font-medium text-highlighted"
                >End <span class="text-error">*</span></label
              >
              <UInput
          ...
          <p v-if="errors.end_time" class="text-xs text-error">
            {{ errors.end_time }}
          </p>
            ...
            <div class="space-y-2">
              <label class="text-sm font-medium text-highlighted">
                ...
                <span class="text-muted text-xs"
                  >(optional — recurs indefinitely if blank)</span
                >
              </label>
              <UInput
                ...
                :disabled="loading"
              />
              <p v-if="errors.ends_on" class="text-xs text-error">
                {{ errors.ends_on }}
              </p>
            </div>
          </div>
          ...
          <div class="flex items-center gap-2">
            ...
            <span class="text-sm text-highlighted"
              >Active (unpins from calendars when off)</span
            >
          </div>
        </form>
      </UModalBody>
