...
//
// A fixed roster of masters students, spanning every Canvas course
// they've been synced into (not just one selected course, unlike the
...
//
// Reads only from server/api/admin/canvas-masters/roster.get.js, which
// itself reads only from already-synced tables -- no live Canvas calls
// happen here. Course data comes from whatever's already been synced
// via the regular Canvas tab's "Sync Now" button; this page doesn't
// re-sync Canvas data itself, only the attendance sheet.
