...
// pages/admin/canvas-masters/index.vue
//
...
// Add/edit-student modal -- entryModalEntry is null for "add new", or
// the { email, program, cohortLabel } of the row being edited.
const entryModalOpen = ref(false)
const entryModalEntry = ref<{ email: string; program: string; cohortLabel: string } | null>(null)
...
function openEditStudent(s: any) {
  entryModalEntry.value = { email: s.email, program: s.program, cohortLabel: s.cohortLabel }
  entryModalOpen.value = true
}

async function handleRemoveStudent(s: any) {
  if (!confirm(`Remove ${s.name || s.email} from the Canvas Masters roster? This only removes them from this roster list -- any Canvas data already synced for them is kept and will reappear if they're re-added.`)) return
  try {
    await $fetch('/api/admin/canvas-masters/roster-entry', { method: 'DELETE', body: { email: s.email } })
    showSuccess('Removed from roster', `${s.name || s.email} has been removed.`)
    await fetchRoster()
  } catch (err: any) {
    showError('Failed to remove', err?.data?.statusMessage || err?.message || 'Please try again.')
  }
}
...
// Existing values offered as quick-pick suggestions in the add/edit
// modal, so admins reuse the same spelling rather than accidentally
// splitting a group ("Data Science" vs "data science").
const existingPrograms = computed(() => [...new Set(students.value.map((s) => s.program).filter(Boolean))].sort())
const existingCohortLabels = computed(() => [...new Set(students.value.map((s) => s.cohortLabel).filter(Boolean))].sort())
</script>
...
<template>
  <UDashboardPanel id="canvas-masters">
    ...
    <template #body>
        ...
        >
          ...
          <div class="flex items-center gap-1 shrink-0">
            <UButton
              ...
              size="xs"
              square
              @click.stop.prevent="openEditStudent(s)"
            />
            <UButton
              ...
              size="xs"
              square
              @click.stop.prevent="handleRemoveStudent(s)"
            />
          </div>
      ...
      <AdminCanvasMastersRosterEntryModal
        v-model:open="entryModalOpen"
        :entry="entryModalEntry"
        :existing-programs="existingPrograms"
        :existing-cohort-labels="existingCohortLabels"
        @saved="fetchRoster"
      />
    </template>
  </UDashboardPanel>
