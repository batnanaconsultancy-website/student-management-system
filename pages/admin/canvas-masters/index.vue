...
const syncStatus = ref<{ canvas: string | null; attendance: string | null }>({
  canvas: null,
  attendance: null,
});
...
async function syncFromCanvas() {
  ...
  try {
    ...
    for (const email of emails) {
      syncProgress.value = {
        stage: "resolving",
        current: syncProgress.value!.current + 1,
        total: emails.length,
        label: email,
      };
      try {
        ...
        if (res?.data?.found) {
          studentsResolved += 1;
          for (const courseId of res.data.courseIds || [])
            courseIdsToSync.add(courseId);
        } else {
    ...
    try {
      await $fetch("/api/admin/canvas-masters/mark-canvas-synced", {
        ...
        body: {
          studentsResolved,
          studentsUnresolved: unresolvedEmails,
          coursesSynced: coursesSynced.map((c) => c.courseId),
        },
      });
    } catch {
      // non-critical, ignore
    ...
    showSuccess(
      ...
      `${studentsResolved} students resolved, ${coursesSynced.length} course${coursesSynced.length === 1 ? "" : "s"} synced` +
        (unresolvedEmails.length > 0
          ? ` (${unresolvedEmails.length} email${unresolvedEmails.length === 1 ? "" : "s"} not found on Canvas)`
          : ""),
    );
    await fetchRoster();
  } catch (err: any) {
...
async function syncAttendanceSheet() {
  ...
  try {
    const res = await $fetch("/api/admin/canvas-masters/attendance-sync", {
      method: "POST",
      body: {},
    });
    showSuccess(
      "Attendance synced",
      `${res.data.studentsSynced} students updated from the sheet`,
    );
    await fetchRoster();
  } catch (err: any) {
    ...
    if (err?.data?.statusCode === 400) {
      const url = window.prompt(
        "Paste the Google Sheet URL to sync attendance from:",
      );
      if (url) {
        try {
          const res = await $fetch(
            "/api/admin/canvas-masters/attendance-sync",
            { method: "POST", body: { sheetUrl: url } },
          );
          showSuccess(
            "Attendance synced",
            `${res.data.studentsSynced} students updated from the sheet`,
          ...
          showError(
            ...
            err2?.data?.statusMessage || err2.message,
          );
        }
      }
    } else {
...
const summary = computed(() => {
  ...
  const cohorts = new Set(
    students.value.map((s) => `${s.program}::${s.cohortLabel}`),
  ).size;
    ...
    <template #body>
      ...
      <div class="flex items-center gap-4 flex-wrap text-xs text-muted mb-4">
        <span class="flex items-center gap-1.5">
          ...
          {{
            syncStatus.canvas
              ? `last synced ${formatSyncDate(syncStatus.canvas)}`
              : "never synced"
          }}
        </span>
        <span class="flex items-center gap-1.5">
          ...
          Attendance:
          {{
            syncStatus.attendance
              ? `last synced ${formatSyncDate(syncStatus.attendance)}`
              : "never synced"
          }}
        </span>
      </div>
      ...
      <!-- Live progress while "Sync From Canvas" is running, since it's
           ...
           a large roster. -->
      <UCard
        v-if="canvasSyncing && syncProgress"
        variant="subtle"
        class="mb-6"
        :ui="{ body: '!py-3' }"
      >
        <p class="text-xs text-muted">
          <UIcon
            ...
            class="size-3.5 animate-spin inline mr-1"
          />
          <template v-if="syncProgress.stage === 'resolving'">
            Resolving students on Canvas… ({{ syncProgress.current }}/{{
              syncProgress.total
            }}{{ syncProgress.label ? ` — ${syncProgress.label}` : "" }})
          </template>
          <template v-else>
            Syncing courses… ({{ syncProgress.current }}/{{ syncProgress.total
            }}{{ syncProgress.label ? ` — ${syncProgress.label}` : "" }})
          </template>
          This runs as several small requests, so it's normal for this to take a
          while for a large roster.
        </p>
      </UCard>
      <UCard
        v-else-if="lastCanvasSyncSummary"
        variant="subtle"
        class="mb-6"
        :ui="{ body: '!py-3' }"
      >
        <p class="text-xs text-muted">
          Last sync just now:
          {{ lastCanvasSyncSummary.studentsResolved }} students resolved,
          {{ lastCanvasSyncSummary.coursesSynced.length }} course{{
            lastCanvasSyncSummary.coursesSynced.length === 1 ? "" : "s"
          }}
          synced
          <template v-if="lastCanvasSyncSummary.studentsUnresolved.length > 0">
            · {{ lastCanvasSyncSummary.studentsUnresolved.length }} email{{
              lastCanvasSyncSummary.studentsUnresolved.length === 1 ? "" : "s"
            }}
            not found on Canvas ({{
              lastCanvasSyncSummary.studentsUnresolved.join(", ")
            }})
          </template>
        </p>
      </UCard>
      ...
      >
        ...
        >
          <div
            class="size-8 rounded-full bg-elevated flex items-center justify-center shrink-0"
          >
          ...
          <UBadge color="neutral" variant="subtle" size="sm"
            >{{ s.program }} · {{ s.cohortLabel }}</UBadge
          >
          <UBadge
            v-if="!s.canvasSynced"
            color="warning"
            variant="subtle"
            size="sm"
            >Not synced yet</UBadge
          >
          <div
            class="hidden sm:flex items-center gap-1.5 text-xs text-muted w-32 shrink-0"
          >
            <UIcon name="i-lucide-video" class="size-3.5" />
            {{ s.attendance.meetingsAttended }} meetings ({{
