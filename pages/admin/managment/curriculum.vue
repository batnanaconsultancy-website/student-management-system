...
const programs = computed(() =>
  (programsData.value?.data || []).map((p: any) => ({ label: p.name, value: p.id }))
)
  ...
  <div class="mt-6 w-full lg:max-w-3xl mx-auto space-y-8">
    ...
    <div>
      ...
      <p class="text-muted text-[15px] mt-1">
        Define new seasons and projects. Once created, schedule them onto cohorts
        via Management → Seasons and Management → Projects.
      </p>
    </div>
        ...
        <div class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            ...
            <UFormField label="Order in Programme">
              <UInput v-model.number="seasonForm.order_in_program" type="number" min="1" />
            </UFormField>
          </div>
          <UFormField label="Programme" required>
            <USelect v-model="seasonForm.program_id" :items="programs" placeholder="Select programme" />
          </UFormField>
        ...
        <div class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            <UFormField label="Project Name" required>
              <UInput v-model="projectForm.name" placeholder="e.g. Building a REST API" />
            </UFormField>
            <UFormField label="Duration (days)">
              <UInput v-model.number="projectForm.duration_days" type="number" min="1" placeholder="e.g. 10" />
            </UFormField>
          </div>
          <UFormField label="Description">
            <UTextarea v-model="projectForm.description" placeholder="Optional project description" :rows="2" />
          </UFormField>
          <div class="grid grid-cols-2 gap-4">
            <UFormField label="Programme" required>
              <USelect
