...
<template>
  <div class="mt-6 w-full lg:max-w-2xl mx-auto gap-6 space-y-8">
    ...
    <div>
      ...
      <UCard variant="subtle" class="mt-4" :ui="{ body: '!py-4 w-full !px-8' }">
        <div class="space-y-4">
          <div>
            ...
            <p class="text-muted text-sm mb-3">
              Enter email addresses to grant admin access. Press Enter to add
              multiple emails.
            </p>
            <UInputTags
              v-model="emails"
          ...
          <UButton
            ...
            block
          >
            Add {{ emails.length > 0 ? emails.length : "" }} Admin{{
              emails.length !== 1 ? "s" : ""
            }}
          </UButton>
        </div>
    ...
    <!-- Current Admins List -->
    <div>
      <h2 class="text-highlighted font-medium text-left w-full">
        Current Admins
      </h2>
      <p class="text-muted text-[15px] text-pretty mt-1">
        People with admin access to this system
      </p>

      <UCard variant="subtle" class="mt-4" :ui="{ body: '!py-4 w-full !px-8' }">
        <div
          v-if="adminLoading && admins.length === 0"
