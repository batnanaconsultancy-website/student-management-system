...
const loadProjectsForSeason = async (seasonId: string) => {
  ...
  schedules.forEach((schedule: any) => {
    ...
    console.log(
      `✅ Adding project to timeline: ${schedule.projects.name} (days ${startDay}-${endDay})`,
    );
  ...
  console.log(
    `🎯 Total timeline projects to display: ${timelineProjects.length}`,
  );
...
const addSeasonsToTimeline = async (
  ...
  for (const seasonGroup of groupedSeasons) {
    ...
    if (isFinalProjectSeason) {
      console.log(
        `🎓 Final Project season -- marker only, no individual projects to fetch`,
      );
      continue;
    }
    ...
    for (const seasonId of seasonGroup.ids) {
      ...
      projectSchedules.forEach((schedule: any) => {
        ...
        console.log(
          `      📌 Project: ${schedule.projects.name} (${schedule.start_date} to ${schedule.end_date})`,
        );
        ...
        while (currentMonth <= lastMonth) {
          ...
          console.log(
            `        ➕ Adding to month ${monthKey}: days ${startDay}-${endDay}`,
          );
  ...
  console.log(
    "✅ Generated timeline projects from actual dates:",
    timelineProjects,
  );
  console.log(
    `📊 Total months with projects: ${Object.keys(timelineProjects).length}`,
  );
...
const loadProjectsForCurrentMonth = () => {
  ...
  console.log(
    `✨ Final projectItems.value (${projectItems.value.length} items):`,
    projectItems.value,
  );
};
...
const calculateNonOverlappingPositions = (projects: any[]) => {
  ...
  return sortedProjects.map((project) => {
    ...
    if (
      assignedRow === 0 ||
      !occupiedRows.find(
        (row) => row.row === assignedRow && row.endDate < project.startDate - 1,
      )
    ) {
