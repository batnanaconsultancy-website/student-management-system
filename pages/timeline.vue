...
const sharedGoogleCalendarId = runtimeConfig.public?.googleCalendarId || null;
...
<template>
  <UDashboardPanel id="calendarNuxtUI">
      ...
      <template #body>
          ...
          <!-- Shared meetings calendar: auto-subscribed at login, but shown
               here too as a fallback (e.g. if login happened before this
               feature existed, or the auto-subscribe call didn't go through)
               and as an embedded view for anyone who'd rather see it inline. -->
          <div v-if="sharedGoogleCalendarId" class="mx-6 mt-4 rounded-lg border border-default bg-elevated/40 p-4">
            <div class="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p class="text-sm font-medium">All meetings also live on Google Calendar</p>
                <p class="text-xs text-muted">You're auto-subscribed when you log in with Google. Not seeing it? Add it manually.</p>
              </div>
              <UButton
                :to="`https://calendar.google.com/calendar/render?cid=${encodeURIComponent(sharedGoogleCalendarId)}`"
                target="_blank"
                external
                size="sm"
                color="neutral"
            ...
            <details class="mt-3">
              <summary class="cursor-pointer text-xs text-muted">Show embedded calendar</summary>
              <iframe
                :src="`https://calendar.google.com/calendar/embed?src=${encodeURIComponent(sharedGoogleCalendarId)}`"
                class="mt-3 h-[500px] w-full rounded-md border border-default"
                frameborder="0"
                scrolling="no"
              />
            </details>
