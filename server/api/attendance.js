...
export default defineEventHandler(async (event) => {
  try {
    ...
    const { data: students, error: studentsErr } = await client
      .from("students")
      .select(
        "id,username,cohort_id,workshops_attended,standup_attended,mentoring_attended",
      );
    ...
    if (studentsErr) {
      ...
      throw createError({
        statusCode: 500,
        statusMessage: studentsErr.message || "Failed fetching students",
      });
    }

    const sumWorkshopAttended = students.reduce(
      (sum, s) => sum + (s.workshops_attended || 0),
      0,
    );
    const sumStandupAttended = students.reduce(
      (sum, s) => sum + (s.standup_attended || 0),
      0,
    );
    const sumMentoringAttended = students.reduce(
      (sum, s) => sum + (s.mentoring_attended || 0),
      0,
    );
    ...
    function perStudent(sum) {
      return studentCount > 0
        ? Math.round((sum / studentCount) * 100) / 100
        : null;
    }
    ...
    const avgOverall = perStudent(
      sumWorkshopAttended + sumStandupAttended + sumMentoringAttended,
    );
