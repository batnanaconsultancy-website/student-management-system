...
export default defineEventHandler(async (event) => {
  try {
    ...
    for (let i = 0; i < body.students.length; i++) {
      ...
      try {
        ...
        if (csvEmails.has(student.email)) {
          errors.push(
            `Row ${i + 2}: Duplicate email "${student.email}" in CSV file`,
          );
          continue;
        }
        if (student.qwasarId && csvUsernames.has(student.qwasarId)) {
          errors.push(
            `Row ${i + 2}: Duplicate Qwasar ID "${student.qwasarId}" in CSV file`,
          );
          continue;
        }
        ...
        // Map program name
        const programName =
          programNameMap[student.programme] || student.programme;
        ...
        if (!cohortId) {
          errors.push(
            `Row ${i + 2}: Cohort "${student.cohort}" with program "${programName}" not found`,
          );
          continue;
        }
    ...
    const usernames = studentsToInsert
      .map((s) => s.username)
      .filter((u) => u !== null);
    ...
    const { data: existingStudents } = await client
      ...
      .or(
        `username.in.(${usernames.join(",")}),email.in.(${emails.join(",")})`,
      );

    const existingUsernames = new Set(
      existingStudents?.map((s) => s.username) || [],
    );
    ...
    studentsToInsert.forEach((student, index) => {
      if (
        existingUsernames.has(student.username) ||
        existingEmails.has(student.email)
      ) {
        skipped.push(
          `Skipped: ${student.first_name} ${student.last_name} (${student.email}) - already exists`,
        );
      } else {
